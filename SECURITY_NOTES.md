...
| `falco` | Falcosidekick (Orion host + per managed env) → POST `/api/monitoring/security/webhooks/falco` | HMAC-SHA256 via `X-Orion-Falco-Signature`, 5-min replay window | `FALCO_WEBHOOK_SECRET` env var, mirrored to Vault KV at `secret/orion/falco/webhook_secret` |
