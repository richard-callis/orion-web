...
import { certificatesTools } from './builtin-tools/certificates.js'
import { helmTools } from './builtin-tools/helm.js'
...
if (GATEWAY_TYPE === 'cluster')   { registerBuiltins(kubernetesTools); registerBuiltins(talosTools); registerBuiltins(knowledgeGraphTools); registerBuiltins(discoveryTools); registerBuiltins(certificatesTools); registerBuiltins(helmTools); registerBuiltins(backupTools); registerBuiltins(storageTools); registerBuiltins(networkTools) }
...
if (GATEWAY_TYPE === 'localhost') {
  ...
  // localhost gateway gets cert-manager, helm, backup, storage and network
  // diagnostics in addition to the full cluster toolset above.
  registerBuiltins(certificatesTools)
  registerBuiltins(helmTools)
