...
export async function GET(req: NextRequest) {
  ...
  // Anchor to UTC midnight so bucket keys (toISOString slice) align with the window
  ...
  for (const m of externalModels) {
    pricingMap.set(m.modelId, {
      ...
      // Use explicit null check — Decimal(0) is an object and truthy, so `? Number()` would misread $0 as unset
      inputPricePer1M:  m.inputPricePer1M  !== null ? Number(m.inputPricePer1M)  : null,
      outputPricePer1M: m.outputPricePer1M !== null ? Number(m.outputPricePer1M) : null,
    })
  }
  ...
  const byDay = []
  // days+1 so the loop includes today (since = day 0, today = day N)
